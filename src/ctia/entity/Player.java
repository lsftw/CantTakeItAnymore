...
public abstract class Player extends Being implements KeyListener {
	...
	protected void postDt() { // prevent moving out of bounds
		if (px < Entity.getMinX()) px = Entity.getMinX();
		if (px + sx > Entity.getMaxX()) px = Entity.getMaxX() - sx;
		if (py < Entity.getMinY()) py = Entity.getMinY();
		if (py + sy > Entity.getMaxY()) py = Entity.getMaxY() - sy;
